...
public class LibraryView {
    ...
    private Integer currentsAdminId = null; // 추가 관리자 DB아이디 DB테이블의 PK
        ...
        while (true) {
            ...
            try {
                switch (choice) {
                    case 1:
                        // 관리자 인가 처리 필요함
                        if (currentsAdminId == null) {
                            System.out.println("관리자만 도서를 추가할수 있습니다.");
                            break;
                        }
                            addBook();
                    ...
                    case 4:
                        if (currentsAdminId == null) {
                            System.out.println("관리자만 학생을 등록할수 있어요.");
                            break;
                        }
                            addStudent();
                        break;
                    case 5:
                        if (currentsAdminId == null){
                            System.out.println("관리자만 학생 목록을 조회할수 있습니다.");
                            break;
                        }
                        listStudents();
                        break;
    ...
    private void addStudent() throws SQLException {
        ...
        service.addStudent(Student.builder().name(name).student_Id(studentId).build());
        ...
        if (students.isEmpty()) {
            ...
            for (Student s : students) {
                System.out.printf("ID: %2d | %-10s | 학번: %s%n",
                        s.getId(), s.getName(), s.getStudent_Id());
            }
        }
    }
    ...
    private void listBorrowedBooks() throws SQLException {
        List<Borrow> borrows = service.borrowsList();
        System.out.println("\n=== 대출 중인 도서 ===");
        if (borrows.isEmpty()) {
            ...
            for (Borrow borrow : borrows) {
                System.out.printf("대출ID: %2d | 도서ID: %2d | 학생ID: %2d | 대출일: %s%n",
                        borrow.getId(), borrow.getBook_Id(),
                        borrow.getStudent_Id(), borrow.getBorrow_Date());
            }
        }
    }
    ...
    public void adminLogin() throws SQLException {
        ...
        if (adminId.isEmpty()) {
            System.out.println("관리자 아이디를 입력해 주세요");
            return;
        }
        System.out.println("관리자 PW: ");
        String password = scanner.nextLine().trim();
        if (password.isEmpty()) {
            System.out.println("관리자 패스워드를 입력해 주세요");
            return;
        }
        ...
        if (admin == null) {
            ...
            currentsAdminId = admin.getId();
            currentAdminName = admin.getName();
            System.out.println(currentStudentName + " 관리자님 환영합니다.");
        }
    }
